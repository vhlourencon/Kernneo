...
public class SubGrupoFiltroPanel  extends GenericFiltroPanel<SubGrupoModel> {
    
    private JComboBox comboBox;
    private JTextField textField;
    private ArrayList<CategoriaModel> listaDeGrupos;

    public SubGrupoFiltroPanel() throws CategoriaException {
    ...
    @Override
    public SubGrupoModel getModelFiltro() {
	if (modelFiltro == null) { 
	    setModelFiltro(new SubGrupoModel());
	    getModelFiltro().setGrupo(null);
	    getModelFiltro().setDescricao(null);
	}
	
	modelFiltro.setDescricao(textField.getText());
	modelFiltro.setGrupo(getGrupoSelecionado());
